...
struct Node* create_local_list(int num_nodes){
	struct Node* head = NULL;
	struct Node* tail = NULL;

	int i;
	for(i = 0; i < num_nodes; i++){
		struct Node* node = generate_data_node();
		if(!node){
  ...
  void * producer_thread( void *arg)
{
    ...
    // Create local list of K nodes (no lock needed)
    struct Node* local_list = create_local_list(K);
    
    if(local_list != NULL)
    {
        // Find the tail of the local list
        struct Node* tail = local_list;
        while(tail->next != NULL) {
            tail = tail->next;
        }
        
        // Single lock acquisition for the entire local list
        pthread_mutex_lock(&mutex_lock);
        
        // Attach the entire local list to the global list
        if( List->header == NULL )
        {
            List->header = local_list;
            List->tail = tail;
        }
        else
        {
            List->tail->next = local_list;
            List->tail = tail;
        }
